...
from src.evaluation import evaluate_auc, evaluate_hits, evaluate_mrr, evaluate_acc, evaluate_rmse
from src.utils import get_num_samples


def get_test_func(use_node_features_only):
    if use_node_features_only:
        return get_preds
    else:
...
def test(model, evaluator, train_loader, val_loader, test_loader, args, device, emb=None, eval_metric='hits'):
    ...
    train_pred, train_true = test_func(model, train_loader, device, args, split='train')
    print("get val predictions")
    val_pred, val_true = test_func(model, val_loader, device, args, split='val')
    print("get test predictions")
    test_pred, test_true = test_func(model, test_loader, device, args, split='test')

    results = {}
    eval_metric = eval_metric.split(',')
    for key in eval_metric:
        if key == 'rmse':
            res = evaluate_rmse(train_pred, train_true, val_pred, val_true, test_pred, test_true)
        results.update(res)
...
def get_preds(model, loader, device, args, split=None):
    ...
    if split == 'train':
        ...
        mask = data.test_mask
    test_nodes = node_ids[mask]
    loader = DataLoader(test_nodes, args.eval_batch_size, shuffle=False)
...
def test_logistic_regression(model, evaluator, train_loader, args, eval_metric):
    data = train_loader.dataset
    train_mask, val_mask, test_mask = data.train_mask, data.val_mask, data.test_mask
    pos_train_pred, neg_train_pred, train_pred, train_true = get_pred_logistic_regression(model, train_loader, train_mask)
    pos_val_pred, neg_val_pred, val_pred, val_true = get_pred_logistic_regression(model, train_loader, val_mask)
    pos_test_pred, neg_test_pred, test_pred, test_true = get_pred_logistic_regression(model, train_loader, test_mask)
    results = {}
    eval_metric = eval_metric.split(',')
    for key in eval_metric:
        if key == 'hits':
...
def test_linear_regression(model, train_loader, args):
    data = train_loader.dataset
    train_mask, val_mask, test_mask = data.train_mask, data.val_mask, data.test_mask
    train_pred, train_true = get_pred_linear_regression(model, train_loader, train_mask)
    val_pred, val_true = get_pred_linear_regression(model, train_loader, val_mask)
    test_pred, test_true = get_pred_linear_regression(model, train_loader, test_mask)
    train_rmse = ((train_pred - train_true)**2).mean().sqrt()
    val_rmse = ((val_pred - val_true)**2).mean().sqrt()
    test_rmse = ((test_pred - test_true)**2).mean().sqrt()
    return train_rmse, val_rmse, test_rmse


def get_pred_logistic_regression(model, loader, mask):
    data = loader.dataset
    # training
    z = data.z[mask]
    ...
    labels = np.array(data.y[mask])
    pred = model.predict_proba(z)
    n_classes = pred.shape[-1]
    if n_classes == 2:
        # only take proba of y=1 class
...
def get_pred_linear_regression(model, loader, mask):
    data = loader.dataset
    # training
    z = data.z[mask]
    # z = torch.cat([z, torch.ones(z.size(0), 1)], dim=-1)
    ...
    return torch.tensor(pred), torch.tensor(labels)