...
from torch_geometric.transforms import BaseTransform, remove_duplicated_edges
from torch_geometric.typing import EdgeType
from torch_geometric.utils import negative_sampling, structured_negative_sampling, structured_negative_sampling_feasible


# biased negative sampling based on torch_geometric.transforms.random_link_split
class BiasedLinkSplit(BaseTransform):
    ...
    def __init__(
        ...
        neg_sample_strategy='biased-random',
    ) -> None:
        ...
        self.neg_sampling_strategy = neg_sample_strategy
    ...
    def forward(
        ...
        for item in zip(stores, train_stores, val_stores, test_stores,
            ...
            # start to sample negative samples
            neg_sample_methods = self.neg_sampling_strategy.split('-')

            if 'biased' in neg_sample_methods:
                # Create biased negative samples: reverse of positive edges
                # first sample biased negative samples: if (i,j) is positive, then includes (j,i) as negative if it does not exist in the original graph
                unidi_edges = mark_unidirectional_edges(edge_index) # find unidirectional edges: only their reverses can server as negative samples
                rev_train_edges = edge_index[:, train_edges].flip(0)
                rev_train_edges = rev_train_edges[:, unidi_edges[train_edges]] # reverse edges that does not exists in the original graphs
                rev_val_edges = edge_index[:, val_edges].flip(0)
                rev_val_edges = rev_val_edges[:, unidi_edges[val_edges]]
                rev_test_edges = edge_index[:, test_edges].flip(0)
                rev_test_edges = rev_test_edges[:, unidi_edges[test_edges]]
                num_rev_train = rev_train_edges.size(-1)
                num_rev_val = rev_val_edges.size(-1)
                num_rev_test = rev_test_edges.size(-1)

                #if self.neg_sampling_strategy == 'biased-random-half':
                #if 'half' in neg_sample_methods:
                    # only take half of the non-existing reverse edges as negative samples
                    #rev_train_edges = rev_train_edges[:, torch.randperm(num_rev_train)[:num_rev_train // 2]]
                    #rev_val_edges = rev_val_edges[:, torch.randperm(num_rev_val)[:num_rev_val // 2]]
                    #rev_test_edges = rev_test_edges[:, torch.randperm(num_rev_test)[:num_rev_test // 2]]


            # other non-exists edges as negative samples
            num_neg_train = 0
            ...
            if self.neg_sampling_strategy == 'biased':
                num_neg_train = num_neg_train - num_rev_train
                num_neg_val = num_neg_val - num_rev_val
                num_neg_test = num_neg_test - num_rev_test
            #elif self.neg_sampling_strategy == 'biased-random-half':
                #num_neg_train = num_neg_train // 2
                #num_neg_val = num_neg_val // 2
                #num_neg_test = num_neg_test // 2
            ...
            # sampling random or structured negative samples
            if 'structured' in self.neg_sampling_strategy and structured_negative_sampling_feasible(edge_index, num_nodes, False):
                if 'structured_s' in neg_sample_methods:
                    neg_edge_index = structured_negative_sampling(edge_index, num_nodes, False)
                    neg_edge_index = torch.cat([neg_edge_index[0][None], neg_edge_index[2][None]], dim=0)
                elif 'structured_t' in neg_sample_methods:
                    neg_edge_index = structured_negative_sampling(edge_index.flip(0), num_nodes, False)
                    neg_edge_index = torch.cat([neg_edge_index[2][None], neg_edge_index[0][None]], dim=0)
                elif 'structured_st' in neg_sample_methods:
                    neg_edge_index_src2x = structured_negative_sampling(edge_index, num_nodes, False)
                    neg_edge_index_x2dst = structured_negative_sampling(edge_index.flip(0), num_nodes, False)
                    neg_src = torch.cat([neg_edge_index_src2x[0], neg_edge_index_x2dst[2]])
                    neg_dst = torch.cat([neg_edge_index_src2x[2], neg_edge_index_x2dst[0]])
                    neg_edge_index = torch.cat([neg_src[None], neg_dst[None]], dim=0)
            elif 'random' in neg_sample_methods:
                neg_edge_index = negative_sampling(edge_index, size,
                                                   num_neg_samples=num_neg,
                                                   method='sparse')
            else:
                raise Exception('fail to sample negative samples!')
            ...
            # combine final negative samples
            # TO DO: in some cases, one may require the structured negative samples to be corresponded to positive edges
            neg_edge_index = neg_edge_index[:, torch.randperm(neg_edge_index.size(1))]

            # Create labels:
            neg_edge_index_train = torch.cat([rev_train_edges, neg_edge_index[:, num_neg_val+num_neg_test:]], dim=-1)
            neg_edge_index_val = torch.cat([rev_val_edges, neg_edge_index[:, :num_neg_val]], dim=-1)
            neg_edge_index_test = torch.cat([rev_test_edges, neg_edge_index[:, num_neg_val:num_neg_val + num_neg_test]], dim=-1)

            # only takes part of the negative samples if needed
            if 'half' in neg_sample_methods or 'third' in neg_sample_methods:
                factor = 2 if 'half' in neg_sample_methods else 3
                neg_edge_index_train = neg_edge_index_train[:, torch.randperm(neg_edge_index_train.size(1))[0:neg_edge_index_train.size(1)//factor]]
                neg_edge_index_val = neg_edge_index_val[:, torch.randperm(neg_edge_index_val.size(1))[0:neg_edge_index_val.size(1)//factor]]
                neg_edge_index_test = neg_edge_index_test[:, torch.randperm(neg_edge_index_test.size(1))[0:neg_edge_index_test.size(1)//factor]]
...
def mark_unidirectional_edges(edge_index):
    num_edges = edge_index.size(-1)
    all_edge_index = torch.cat([edge_index, edge_index.flip(0)], dim=-1)
    unique, inverse, counts = torch.unique(all_edge_index, dim=-1, return_inverse=True, return_counts=True)
    return (counts[inverse] == 1)[0:num_edges]

def remove_bidirectional_edges(edge_index):
    num_edges = edge_index.size(-1)
    all_edge_index = torch.cat([edge_index, edge_index.flip(0)], dim=-1)
    unique, inverse, counts = torch.unique(all_edge_index, dim=-1, return_inverse=True, return_counts=True)
    mask = (counts[inverse] == 1)[0:num_edges] # only keeps unidirectional edges
