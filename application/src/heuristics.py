...
def PPR(A, edge_index, bidirection=False):
    """
    ...
    _, inverse = torch.sort(sort_indices)
    ...
    if not bidirection:
        A = A + A.T
    print('Calculating Personalized Pageranks...')
    for i in tqdm(range(edge_reindex.shape[1])):
        ...
        if bidirection:
            ppr_rev = pagerank_power(A.T, p=0.85, personalize=personalize, tol=1e-7)
        else:
            ppr_rev = None
        j = i
        ...
        cur_scores_rev = ppr_rev[all_dst] if ppr_rev is not None else None
        if cur_scores.ndim == 0:
            cur_scores = np.expand_dims(cur_scores, 0)
            cur_scores_rev = np.expand_dims(cur_scores_rev, 0)
        if bidirection:
            cur_scores = np.concatenate((cur_scores[:, None], cur_scores_rev[:, None]), axis=-1)
        scores.append(np.array(cur_scores))

    scores = np.concatenate(scores, 0)
    scores = scores[inverse]
    # inverse to original order
    print(f'evaluated PPR for {len(scores)} edges')
    return torch.FloatTensor(scores).view([len(scores), -1])
