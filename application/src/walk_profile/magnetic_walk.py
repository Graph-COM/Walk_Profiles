...
from src.walk_profile.message_passing import message_passing, bidirectional_message_passing
from torch_geometric.utils import degree as pyg_degree
import time



def get_magnetic_walks(edge_index, num_nodes, q, walk_length, source_node=None, normalize=False, history=True):
...
def get_magnetic_walks_edge(edge_index, num_nodes, q, walk_length, source_node=None, normalize=False, nbt=False, history=True):
...
def compute_magnetic_walks(edge_index, num_nodes, q, walk_length, source_node=None, nbt=False, normalize=False, history=True):
    # ultimate interface for computing magnetic walks
    #if nbt and normalize:
    if nbt:
        return get_magnetic_walks_edge(edge_index, num_nodes, q, walk_length, source_node=source_node, normalize=normalize, nbt=nbt, history=history)
    else:
        return get_magnetic_walks(edge_index, num_nodes, q, walk_length, source_node=source_node, normalize=normalize, history=history)
