...
class SEALDataset(InMemoryDataset):
    def __init__(self, root, data, pos_edges, neg_edges, num_hops, percent=1., split='train',
                 ...
                 max_nodes_per_hop=None, max_dist=1000, directed=False, seed=None,
                 save_no_subgraph=False, mw_params=None):
        self.root = root
        self.data = data
    ...
    def processed_dir(self):
        return os.path.join(self.root, 'processed_seed' + str(self.seed))
    ...
    def process(self):
        ...
        if self.save_no_subgraph:
            # only save link features in disk
            z=torch.cat([e[None] for e in pos_list + neg_list], dim=0)
            z=torch.tensor(np.array(z)) # avoid unexpected extra memory in torch.tensor
...
def get_train_val_test_datasets(dataset, train_data, val_data, test_data, args):
    ...
    sample = 'all'
    #neg_sample_method = 'bias' if args.biased_neg_sample else ''
    neg_sample_method = args.neg_sample
    ...
    if args.node_label in ['mw', 'rw', 'rw+', 'wp']:
        ...
        node_label_name += '-compact' if args.compact_q else ''
    ...
    path = f'{dataset.root}_seal_{sample}_hops_{args.num_hops}_neg_{neg_sample_method}_label_{node_label_name}_maxdist_{args.max_dist}_mnph_{args.max_nodes_per_hop}'
    ...
    # TO DO: when drop subgraph, transform this dataset into RegularDataset for efficient training
...
def construct_pyg_graph(node_ids, adj, dists, node_features, y, node_label='drnl', max_dist=1000, src_degree=None,
                        dst_degree=None, save_no_subgraph=False, mw_params=None):
    """
    ...
    if node_label == 'drnl':  # DRNL
        ...
        z = cn_node_labeling(adj, 0, 1)
    elif node_label == 'cn+':  # common neighbor w/ direction
        z = cn_plus_node_labeling(adj, 0, 1)
    elif node_label == 'rw':
        z = rw_node_labeling(adj, 0, 1, max_dist, mw_params) # for walk-based method, max_dist refers to the walk length to consider
    elif node_label == 'rw+':
    ...
    elif node_label == 'zero':
        z = torch.zeros(len(dists), dtype=torch.long)
    else:
        raise Exception('Unrecognized node labeling during preprocessing!')

    if save_no_subgraph:
        # when we only cares about the labeling of target links: just return link features
...
def extract_enclosing_subgraphs(link_index, A, x, y, num_hops, node_label='drnl',
    ...
    for src, dst in tqdm(link_index.tolist()):
        ...
        if directed: # get total degree
            temp = get_src_dst_degree(src, dst, A.T, max_nodes_per_hop)
            src_degree += temp[0]
            dst_degree += temp[1]
        tmp = k_hop_subgraph(src, dst, num_hops, A, ratio_per_hop,
                             max_nodes_per_hop, node_features=x, y=y,
                             directed=directed, A_csc=A_csc)
        data = construct_pyg_graph(*tmp, node_label, max_dist, src_degree, dst_degree, save_no_subgraph, mw_params)
